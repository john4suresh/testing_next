...
import { professionalIndustryExperience } from "@/lib/apiUrls";

const ProfessionalIndustryExperience = () => {
  const { data, error, isLoading, mutate, isValidating } = useSWR(
    `${process.env.NEXT_PUBLIC_BACKEND_URL}${professionalIndustryExperience}`
  );
  ...
  const handleClick = async () => {
    ...
    try {
      let response = await api.put(professionalIndustryExperience, {
        professional_industries: selectedChoices,
      });
  ...
  return (
    <div className="container mx-auto max-w-5xl px-5">
      <h1 className="mb-8 text-center text-[34px] font-semibold">
        Select your areas of professional industry experience
      </h1>
      <p className="text-base font-normal">
        Please select all industries in which you have personally held a
...
export default ProfessionalIndustryExperience;
